...
import { CalendarIcon, Loader2, ArrowLeft, Building2, LayoutGrid } from 'lucide-react';
...
const PALETTE = [
  { bg: '#eef2f7', text: '#3a5275', dot: '#7a9bbf', border: '#b8cfe0' },
  { bg: '#f0f4f0', text: '#3d5c3d', dot: '#85a885', border: '#b8d0b8' },
  { bg: '#f5f0eb', text: '#6b4f35', dot: '#b8967a', border: '#d9c0aa' },
  { bg: '#f4f0f5', text: '#5a3d6b', dot: '#a080b8', border: '#c8b0d8' },
  { bg: '#f5f0f0', text: '#6b3d3d', dot: '#b88080', border: '#d8b0b0' },
  { bg: '#eff4f5', text: '#2d5a5c', dot: '#70a0a5', border: '#aacdd0' },
  { bg: '#f2f2ee', text: '#4d4d35', dot: '#9a9870', border: '#c5c4a5' },
...
export default function NovoLancamento() {
  ...
  useEffect(() => {
    (async () => {
      const [c, h, s, v] = await Promise.all([
        ...
        supabase.from('tabela_valores').select('*').eq('ativo', true),
      ]);
      setCooperados(c.data ?? []);
      setHospitals(h.data ?? []);
      setSectors(s.data ?? []);
      if (v.error) toast.error('Erro ao carregar tabela de valores: ' + v.error.message);
  ...
  const hospitalSelecionado = hospitals.find(h => h.id === form.hospital_id);
  const setorSelecionado = sectors.find(s => s.id === form.setor_id);

  const hospitalColorMap = useMemo(() => {
    const map: Record<string, typeof PALETTE[0]> = {};
    hospitals.forEach((h, i) => { map[h.id] = PALETTE[i % PALETTE.length]; });
    return map;
  }, [hospitals]);

  const hospitalColor = form.hospital_id ? hospitalColorMap[form.hospital_id] : null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Verificar duplicatas: mesmo cooperado + mesma data + mesmo tipo de plantão
    const datasStr = datasPlantao.map(d => format(d, 'yyyy-MM-dd'));
    const { data: existentes } = await supabase
      .from('lancamentos_plantoes')
      .select('data_plantao, tipo_plantao')
      .eq('cooperado_id', form.cooperado_id)
      .eq('tipo_plantao', form.tipo_plantao)
      .in('data_plantao', datasStr);

    if (existentes && existentes.length > 0) {
      const datasRepetidas = existentes.map(e =>
        format(new Date(e.data_plantao + 'T12:00:00'), "dd/MM/yyyy", { locale: ptBR })
      );
      setSubmitting(false);
      toast.error(
        `Lançamento duplicado! ${cooperadoSelecionado.nome} já possui plantão ${form.tipo_plantao} em: ${datasRepetidas.join(', ')}`
      );
      return;
    }
    ...
    if (savedCount !== inserts.length) {
      toast.error(`Apenas ${savedCount} de ${inserts.length} plantões foram salvos. Verifique e tente novamente.`);
      return;
    }
    toast.success(savedCount > 1 ? `${savedCount} plantões registrados com sucesso` : 'Plantão registrado com sucesso');
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <Select value={form.tipo_plantao} onValueChange={v => setTipoPlantao(v as 'normal' | 'extra' | 'diarista')}>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={v => setForm(f => ({ ...f, hospital_id: v, setor_id: '' }))}
                >
                  <SelectTrigger
                    style={hospitalColor ? {
                      borderColor: hospitalColor.dot,
                      borderWidth: '2px',
                      backgroundColor: hospitalColor.bg,
                      color: hospitalColor.text,
                      fontWeight: 600,
                    } : {}}
                  >
              ...
              <div>
                ...
                >
                  <SelectTrigger
                    style={hospitalColor && form.setor_id ? {
                      borderColor: hospitalColor.dot,
                      borderWidth: '2px',
                      backgroundColor: hospitalColor.bg,
                      color: hospitalColor.text,
                      ...
                      borderColor: hospitalColor.border,
                    } : {}}
                  >
                    <SelectValue placeholder={form.hospital_id ? 'Selecione…' : 'Escolha um hospital primeiro'} />
                  </SelectTrigger>
                  <SelectContent>
                    {setoresDoHospital.map(s => (
                      <SelectItem key={s.id} value={s.id}>
                        <span className="flex items-center gap-2">
                          <span
                            className="inline-block w-3 h-3 rounded-full flex-shrink-0"
                            style={{ backgroundColor: hospitalColor?.dot ?? '#94a3b8' }}
                          />
                          {s.nome}
                        </span>
                      </SelectItem>
              ...
              {/* Banner de confirmação de destino */}
              {hospitalSelecionado && (
                ...
                >
                  <p className="text-[11px] font-bold uppercase tracking-widest mb-2"
                    style={{ color: hospitalColor?.dot }}>
                    Destino do lançamento
                  </p>
                  <div className="flex items-start gap-4">
                    <div className="flex items-center gap-2 flex-1">
                      <Building2 className="h-5 w-5 flex-shrink-0" style={{ color: hospitalColor?.dot }} />
                      <div>
                        <p className="text-[11px] text-muted-foreground">Cliente</p>
                        <p className="font-bold text-base leading-tight" style={{ color: hospitalColor?.text }}>
                    ...
                    {setorSelecionado && (
                      <>
                        <div className="w-px self-stretch" style={{ backgroundColor: hospitalColor?.dot, opacity: 0.3 }} />
                        <div className="flex items-center gap-2 flex-1">
                          <LayoutGrid className="h-5 w-5 flex-shrink-0" style={{ color: hospitalColor?.dot }} />
                          <div>
                    ...
                    {!setorSelecionado && (
                      <p className="text-xs italic self-center" style={{ color: hospitalColor?.dot, opacity: 0.7 }}>
                        Selecione o setor →
                      </p>
                    )}
                  </div>
