...
export default function TabelaValores() {
  ...
  const [novo, setNovo] = useState({
    ...
    tipo_plantao: 'normal' as 'normal' | 'extra',
  ...
  const adicionar = async () => {
    ...
    const { error } = await supabase.from('tabela_valores').insert({
      ...
      valor_hora_cooperado: valorHoraCoopNum,
      ativo: true,
    } as never);
    setSaving(false);
    ...
    setOpen(false);
    setNovo({ profissao: 'enfermeiro', tipo_plantao: 'normal', hospital_id: '__padrao__', valor_hora_cliente: '', valor_hora_cooperado: '' });
    load();
  };
